...
getGenesetCount <- function(enzid, geneset){
  results <- sapply(geneset, function(gs) sum(!is.na(fastmatch::fmatch(gs, enzid))))
  return(results)
...
#' @param snv.table SNV table with sample(sample id), varid (unique variant id), entrez_id(entrez id) and type (type of snv)
#' @param geneset gene set object
#' @keywords GSBurden
#' @export
getSNVGSMatrix <- function(snv.table, geneset){
  ...
  for(snvtype in snvtypes){
    ...
    for(gs in 1:length(geneset)){
      ...
      if(nrow(snvtmp) == 0){
        snvCount[, sprintf("%s_%s", names(geneset)[gs], snvtype)] <- 0
      }else{
        count <- data.frame(table(snvtmp$sample[!duplicated(snvtmp$varid)]))
        names(count) <- c("sample", sprintf("%s_%s", names(geneset)[gs], snvtype))
        snvCount <- merge(snvCount, count, by = "sample", all.x = T)
      }
    }
    
    snvCount[is.na(snvCount)] <- 0
    
    if(nrow(all.out) == 0){
      all.out <- snvCount
    }else{
      all.out <- merge(all.out, snvCount, by = "sample", all = T)
    }
...
SNVGlobalTest <- function(snv.matrix, label, covariates, correctSNVCount = F, standardizeCoefficient = T){
  ...
  distinct.prefixes <- gsub("Total_", "", distinct.prefixes)
  ...
  for(snvtype in distinct.prefixes){
    features <- paste(c("Total"), snvtype, sep="_")
...
SNVBurdenTest <- function(snv.matrix, geneset, label, covariates, correctGlobalBurden = T, correctTotal = T, standardizeCoefficient = T,
  ...
  for(snvtype in distinct.prefixes){
    for(this.gs in names(geneset)){
      out.message <- sprintf("Testing %s", this.gs)
      if(length(distinct.prefixes) > 1){
        out.message <- sprintf("%s for %s SNVs", out.message, snvtype)
      }
      message(out.message)
      ...
      if(permutation){
        for(iperm in 1:nperm){
          ...
          names(ano.perm)[length(names(ano.perm))] <- "pvalue"
          coeff <- add.perm.model$coefficients[feature]
          perm.test.pvalues <- rbind(perm.test.pvalues, data.frame("snvtype" = snvtype, "pvalue" = ano.perm$pvalue[2], coeff))
        }
      }
    }
  }
  ...
  if(permutation){
    ...
    for(i in 1:nrow(test.out)){
      rec <- test.out[i, ]
      this.perm <- perm.test.pvalues[perm.test.pvalues$snvtype == rec$type, ]
